...
def given_card_name_get_attrs(cardName):
    ...
    if attr_dict == {}:
        print(cardName, 'is not the full name of a Yu-Gi-Oh! card, please check your spelling and try again.')
        exit()
    return attr_dict
...
def main():
    while True:
        mode = input('\nWhich mode would you like to use? Type in the corresponding number and press enter or press q to quit.\n\n1 Banish card A from hand, reveal card B from deck, add card C to hand ... I have cards A and C and want to find B.\n2 I want to know all cards that share extacly one thing (ATK, DEF, Level, Type, Attribute) with a card.\n')

        if mode == '1':
            card1 = input('Enter the name of the card in your hand: ')
            card2 = input('Enter the name of the card you want to search: ')

            find_bridge(card1, card2)
        elif mode == '2':
            card = input('Enter the name of the card to find all Small World compatible cards: ')
